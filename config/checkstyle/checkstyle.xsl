<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="checkstyle">
        <html>
            ...
            <body>
                ...
                <xsl:apply-templates select="file[@name and generate-id(.) = generate-id(key('files', @name)) and count(key('files', @name)/error) > 0]" />
    ...
    <xsl:template match="checkstyle" mode="filelist">
        ...
        <table class="log" border="0" cellpadding="5" cellspacing="2" width="100%">
            ...
            <xsl:for-each select="file[@name and generate-id(.) = generate-id(key('files', @name)) and count(key('files', @name)/error) > 0]">
